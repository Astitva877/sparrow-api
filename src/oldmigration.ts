...
// import { HubMigration } from "migrations/hub.migration";
import { OldHubMigration } from "migrations/oldHub.migration";
...
@Module({
  ...
  providers: [databaseProvider, OldHubMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(OldHubMigration);
